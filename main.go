...
func usage() {
	fmt.Fprintf(os.Stderr, "usage: benchdiff [options] old.txt [new.txt] [more.txt ...]\n")
...
var (
	...
	flagPatterns  = flag.String("pat", "", "comma separated two patterns to match benchmarks")
	flagName      = flag.String("name", "Benchmark", "the benchmark name")
)
...
func main() {
	...
	var pat1, pat2 *regexp.Regexp
	if *flagPatterns != "" {
		patterns := strings.Split(*flagPatterns, ",")
		if len(patterns) != 2 {
			flag.Usage()
		}
		var err1, err2 error
		pat1, err1 = regexp.Compile(patterns[0])
		pat2, err2 = regexp.Compile(patterns[1])
